...
export default function EditPostForm({ post }: EditPostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    setIsPending(true);
    // updatePost will handle redirect
  };
    ...
    <div className="h-screen bg-white flex flex-col font-sans overflow-hidden">
      ...
      <header className="h-16 px-6 flex items-center justify-between border-b border-slate-50 bg-white z-20 shrink-0">
        <div className="flex items-center space-x-4">
          <Link href={`/articles/${post.id}`} className="p-2 hover:bg-slate-50 rounded-full transition-colors text-slate-400 hover:text-slate-600 flex items-center space-x-2">
